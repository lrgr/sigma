import numpy as np
from MMMFrozenEmissions import MMMFrozenEmissions
from HMM import HMM
...
class SigMa:
    ...
    def fit(self, seqs, max_iterations=1e8, stop_threshold=1e-3):
        ...
        for iteration in range(int(max_iterations)):
            ...
            # expected_g, _ = self.mmm.expectation_step(gmm_seqs_counts)
    ...
    def log_probability(self, seqs):
        ...
        if len(hmm_seqs) == 0:
            return self.mmm.log_probability(gmm_seqs)

        return self.mmm.log_probability(gmm_seqs) + self.hmm.log_probability(hmm_seqs)
    ...
    def predict(self, seqs):
        ...
        for seq in seqs:
            ...
            current_none_special_state_viterbi = []
            for i in range(len(seq)):
                if current_cloud_indicator[i] == self.num_states:
                    current_none_special_state_viterbi.append(gmm_most_probable_state[seq[i]])
                    current_cloud_indicator[i] = 0
                else:
                    current_none_special_state_viterbi.append(current_cloud_indicator[i])
                    current_cloud_indicator[i] = 1

            cloud_indicator.append(current_cloud_indicator)
            none_special_state_viterbi.append(current_none_special_state_viterbi)

        return cloud_indicator, none_special_state_viterbi
